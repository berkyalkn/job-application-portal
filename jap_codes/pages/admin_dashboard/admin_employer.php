...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['edit'])) {
        ...
        $updateQuery = "UPDATE users SET email = '$email', first_name = '$first_name', last_name = '$last_name', 
                        ...
                        country = '$country', city = '$city' WHERE username = '$username' AND role = 'employer'";

        if (mysqli_query($conn, $updateQuery)) {
            echo "Employer information updated successfully!";
        } else {
            echo "Error updating employer information: " . mysqli_error($conn);
        }
    }
    ...
    if (isset($_POST['delete'])) {
        ...
        $deleteQuery = "DELETE FROM users WHERE username = '$username' AND role = 'employer'";
...
<head>
    ...
    <link rel="stylesheet" href="../../assets/css/admin/admin_employer.css">
</head>
<body>
...
<div class="container">
        ...
        <?php while ($row = mysqli_fetch_assoc($result)) { ?>
            <div class="employer">
                ...
                <form method="POST" style="display:inline;">
                    ...
                    <button type="submit" name="delete" onclick="return confirm('Are you sure you want to delete this employer?');">Delete</button>
                </form>
            </div>
        <?php } ?>
    </div>
    ...
    <div id="edit-form" style="display:none;">
        <h2>Edit Employer Information</h2>
        <form method="POST">
...
<script>
    function populateEditForm(employer) {

        document.getElementById('username').value = employer.username;
        document.getElementById('email').value = employer.email;
        document.getElementById('first_name').value = employer.first_name;
        document.getElementById('last_name').value = employer.last_name;
        document.getElementById('gender').value = employer.gender;
        document.getElementById('date_of_birth').value = employer.date_of_birth;
        document.getElementById('phone_number').value = employer.phone_number;
        document.getElementById('country').value = employer.country;
        document.getElementById('city').value = employer.city;
